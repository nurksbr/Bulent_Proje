...
const TekilUrunFaturaPDF = ({ urun }: { urun: Urun }) => {
  ...
  const fiyat = parseFloat(urun.fiyat.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' }).replace('₺', '').replace(',', ''));
...
export default function AgTeknolojileri() {
  ...
  const fetchUrunler = async () => {
    try {
      const q = query(collection(db, "products"), where("kategori", "==", "Ağ Teknolojileri"));
      ...
      <main className="flex-1 p-8">
        ...
        <div className="bg-white rounded-xl p-6 shadow mb-8">
          <div className="flex justify-between items-center mb-4">
            <div className="font-semibold text-lg">Ağ Teknolojileri Ürünleri</div>
            <button
